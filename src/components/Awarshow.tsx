  ...
  return (
    <div className="flex flex-wrap justify-center gap-5">
      {item.map((v, index) => (
        <div
          key={v.id}
          className="group relative cursor-pointer overflow-hidden border shadow-xl"
        >
          <Image
            src={v.img}
            alt={v.desc}
          ...
          />
          <div
            className="absolute left-0 top-[100%] h-full w-full space-y-6 bg-[rgba(0,0,0,0.5)] p-3 duration-.5 group-hover:top-0"
            onClick={() => {
              setPhotoIndex(index);
              setIsOpen(true);
              setIsLoaded(false);
